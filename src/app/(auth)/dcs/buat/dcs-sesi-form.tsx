...
export function DcsSesiForm({ jabatan, accessToken }: Props) {
  ...
  const {
    ...
    defaultValues: { min_responden: 6, max_responden: 8 },
  });
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      const { data, error, response } = await client.POST("/api/v1/dcs/sesi", {
        body: {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 pt-2">
        ...
        <Link href="/dcs" className="text-sm text-gray-500 hover:text-gray-700">
          Batal
        </Link>
      </div>
    </form>
