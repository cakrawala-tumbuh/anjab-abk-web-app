...
async function fetchData(accessToken: string | undefined, offset: number) {
  ...
  const [uraianRes, pokokRes] = await Promise.all([
    client.GET("/api/v1/task-inventory/uraian-tugas", {
...
export default async function UraianTugasPage({ searchParams }: Props) {
  ...
  const { uraianTugas, total, tugasPokok } = await fetchData(session?.accessToken, offset);
  const pokokMap = Object.fromEntries(tugasPokok.map((tp) => [tp.id, tp.nama]));

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500 dark:text-gray-400">{total} uraian tugas terdaftar</p>
        <Link
