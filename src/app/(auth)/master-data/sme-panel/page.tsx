...
async function fetchAll(accessToken: string | undefined, offset: number) {
  ...
  const [panelRes, jabatanRes] = await Promise.all([
    client.GET("/api/v1/sme-panel", { params: { query: { limit: UKURAN_HALAMAN, offset } } }),
...
export default async function SMEPanelPage({ searchParams }: Props) {
  const session = await auth();
  if (!isAdmin(session)) notFound();

  const sp = await searchParams;
  const offset = offsetHalaman(sp, "hlm");
  const { panels, total, jabatan, gagalPendukung } = await fetchAll(session?.accessToken, offset);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500 dark:text-gray-400">{total} panel SME terdaftar</p>
        <Link
      ...
      {total === 0 ? (
        ...
        <>
          ...
          <Pagination
            ...
            paramKey="hlm"
            basePath="/master-data/sme-panel"
            searchParams={sp}
          />
        </>
      )}
