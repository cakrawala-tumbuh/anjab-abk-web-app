...
import type { JabatanRead, PartisipanRead, WcpRespondenRead, WcpSesiRead } from "@/lib/api/schema";
...
export const metadata = { title: "Detail Sesi WCP — ANJAB-ABK" };
...
async function fetchPageData(accessToken: string | undefined, sesiId: string) {
  ...
  const [sesiRes, respondenRes, jabatanRes, partisipanRes] = await Promise.all([
    client.GET("/api/v1/wcp/sesi/{sesi_id}", { params: { path: { sesi_id: sesiId } } }),
    client.GET("/api/v1/wcp/sesi/{sesi_id}/responden", { params: { path: { sesi_id: sesiId } } }),
...
export default async function WcpSesiDetailPage({ params }: Props) {
                  ...
                  <tr key={r.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap items-center gap-3">
                        {r.sudah_submit && (
                          <Link
                            href={`/wcp/${sesi.id}/hasil-responden/${r.id}`}
