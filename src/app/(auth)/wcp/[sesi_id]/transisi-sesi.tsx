...
export function TransisiSesi({ sesi, accessToken }: Props) {
  ...
  async function doTransisi(aksi: "buka" | "tutup") {
    ...
    try {
      ...
      const path =
        aksi === "buka" ? "/api/v1/wcp/sesi/{sesi_id}/buka" : "/api/v1/wcp/sesi/{sesi_id}/tutup";
  ...
  async function doHapus(paksa: boolean) {
    const pesan = paksa
      ? "Hapus PAKSA sesi ini beserta SELURUH responden & jawabannya? " +
    ...
    try {
      const client = withServerAuth(accessToken);
      const { error: apiError, response } = await client.DELETE("/api/v1/wcp/sesi/{sesi_id}", {
    ...
    <div className="flex flex-wrap items-center gap-3 rounded-lg border border-gray-200 bg-gray-50 px-4 py-3">
      ...
      {sesi.status !== "DRAFT" && (
        <div className="mt-1 w-full border-t border-red-200 pt-3 dark:border-red-900">
          ...
          >
            Hapus paksa sesi ini — SELURUH responden & jawaban ikut terhapus permanen
          </button>
        </div>
      )}
    </div>
