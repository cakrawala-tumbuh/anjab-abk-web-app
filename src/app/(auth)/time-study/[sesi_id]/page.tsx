...
type TsRespondenRead = components["schemas"]["TsRespondenRead"];
type JabatanRead = components["schemas"]["JabatanRead"];
type PartisipanRead = components["schemas"]["PartisipanRead"];

export const metadata = { title: "Detail Sesi Time Study — ANJAB-ABK" };
...
async function fetchPageData(accessToken: string | undefined, sesiId: string) {
  ...
  const [sesiRes, respondenRes, jabatanRes, partisipanRes] = await Promise.all([
    client.GET("/api/v1/time-study/sesi/{sesi_id}", { params: { path: { sesi_id: sesiId } } }),
    client.GET("/api/v1/time-study/sesi/{sesi_id}/responden", {
  ...
  return {
    sesi: sesiRes.data as TsSesiRead,
    responden: (respondenRes.data ?? []) as TsRespondenRead[],
    jabatan: (jabatanRes.data?.items ?? []) as JabatanRead[],
    partisipan: (partisipanRes.data?.items ?? []) as PartisipanRead[],
  };
}

export default async function TimeStudySesiDetailPage({ params }: Props) {
