...
export const metadata = { title: "Sesi Time Study — ANJAB-ABK" };
...
async function fetchPageData(accessToken: string | undefined) {
  ...
  const [sesiRes, jabatanRes] = await Promise.all([
    client.GET("/api/v1/time-study/sesi", { params: { query: { limit: 100 } } }),
  ...
  return {
    sesi: (sesiRes.data.items ?? []) as TsSesiRead[],
    jabatan: (jabatanRes.data?.items ?? []) as JabatanRead[],
  };
}

export default async function TimeStudySesiPage() {
